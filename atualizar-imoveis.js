...
function parseCSV(texto) {
  ...
  const map = {
    ...
    financiamento: cols.findIndex(c => c.includes("financiamento")),
  ...
  for (let i = hi + 1; i < linhas.length; i++) {
    ...
    out.push({
      ...
      financiamento: (get(map.financiamento)||"").toLowerCase().startsWith("s") ? true : ((get(map.financiamento)||"").toLowerCase().startsWith("n") ? false : null),
      descricao,
