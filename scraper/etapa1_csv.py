...
def is_valid_imovel_id(id_str):
    """Verifica se o ID e um numero de imovel valido."""
    s = str(id_str).strip()
    if len(s) < 3:
        return False
    if not s[0].isdigit():
...
def download_csv_estado(estado):
    ...
    try:
        ...
        if id_col is None:
            logger.debug(f"Estado {estado}: coluna ID nao reconhecida em {list(df.columns[:5])}")
            return None
